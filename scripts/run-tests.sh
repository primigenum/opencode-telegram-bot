...
for f in $(find tests -name "*.test.ts" | sort); do
  ...
  # skip: response-streamer, edge-tts and tool-call-streamer use accelerateTime / fakeTimers
  ...
  if echo "$f" | grep -qE "response-streamer|edge-tts|tool-call-streamer"; then
